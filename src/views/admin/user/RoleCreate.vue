<template>
  <div style="text-align: left">
    <el-button class="add-button" type="success" @click="dialogFormVisible = true"
      >添加角色</el-button
    >
    <el-dialog title="添加角色" v-model="dialogFormVisible" @close="clear" width="25%">
      <el-form
        :model="roleForm"
        :rules="rules"
        label-position="left"
      ...
      this.$http
        ...
        .then(resp => {
          if (resp.data.code === 200) {
            ElMessage.success(resp.data.result, "提示", { confirmButtonText: "确定" });
            this.clear();
            this.$emit("onSubmit");
          } else {
            ElMessage.error(resp.data.message, "提示", { confirmButtonText: "确定" });
          }
        })
        .catch(failResponse => {});
