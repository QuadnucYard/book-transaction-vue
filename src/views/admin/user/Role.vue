<template>
  <div>
    <el-dialog title="修改角色信息" v-model="dialogFormVisible">
        ...
        <el-table-column label="状态" width="100">
          <template #default="{ row }">
            <el-switch
              ...
              @change="value => commitStatusChange(value, row)">
            </el-switch>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    listPerms() {
      var _this = this;
      this.$http.get("/admin/role/perm").then(resp => {
    ...
    listMenus() {
      var _this = this;
      this.$http.get("/admin/role/menu").then(resp => {
        if (resp && resp.data.code === 200) {
    ...
    onSubmit(role) {
      ...
      this.$http
        ...
        .then(resp => {
          if (resp && resp.data.code === 200) {
            ElMessage.success(resp.data.result);
            this.dialogFormVisible = false;
            this.listRoles();
          }
        });
      this.$http
        .put("/admin/role/menu?rid=" + role.id, {
